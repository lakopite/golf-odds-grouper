...
branch: main
path: docs/FRONTEND-SPEC.md, frontend/scoreboard/
...
- Standings rule ported from frontend/lib.js verbatim (tiered golfer ranks, lexicographic vector compare, ties as ties).
...
## Shipped
The wireframe was built for real data as `frontend/scoreboard/`, which is now what
`bundle_frontend.py` produces by default. `frontend-design/README.md` maps each screen
and each hard-coded value onto where it comes from in the result JSON.
